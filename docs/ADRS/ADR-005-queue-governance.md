...
Per-tenant policies live in a new tenant_policies table with JSON fields (allowed_queues, banned_patterns, policy_snapshot) and feature flags (allow_heavy, allow_gpu). Results are cached in Redis (policy:<tenant>). When policies change, operators flush the cache key to refresh decisions.
...
- API responses now include queue_name, task_type, policy metadata, and requested_priority to aid clients.
- Additional migrations create tenant_policies and extend the jobs table with governance fields.
