...
def create_app(config_overrides: dict[str, Any] | None = None) -> Flask:
    ...
    app.extensions["calculator_metrics"] = MetricsStore()
    ...
    @app.get("/readyz")
    def ready() -> tuple[dict[str, str], int]:  # pragma: no cover - trivial
        return {"status": "ready"}, HTTPStatus.OK

    @app.get("/metrics")
    def metrics() -> Response:  # pragma: no cover - exercised via integration test
        payload = app.extensions["calculator_metrics"].render()
        return Response(
