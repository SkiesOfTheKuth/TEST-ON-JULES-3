...
4. Ensure `/readyz` continues to pass (it executes a self-test) before rotating traffic.
5. Consider scaling horizontally or adding queue if sustained load continues.
...
- Metrics: scrape `/metrics` for Prometheus-compatible counters/histograms (`calculator_http_requests_total`, `calculator_http_request_duration_seconds`, `calculator_evaluations_total`). Set `PROMETHEUS_MULTIPROC_DIR` to a shared directory when running multiple Gunicorn workers and ensure the scrape target is network-restricted.
